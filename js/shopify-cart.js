...
const queryParts = titles.map((title, i) => `p${i}: products(first: 1, query: "title:'${title.replace(/'/g, "\\'")}'") { edges { node { title variants(first: 100) { edges { node { id availableForSale price { amount currencyCode } selectedOptions { name value } } } } } } }`).join(’\n’);
...
============================================================ */
const CART_FRAGMENT = `id checkoutUrl totalQuantity cost { subtotalAmount { amount currencyCode } } lines(first: 50) { edges { node { id quantity merchandise { ... on ProductVariant { id title price { amount currencyCode } product { title featuredImage { url altText } } selectedOptions { name value } } } } } }`;

async function createCart(variantGid) {
const { data } = await gql(`mutation cartCreate($variantId: ID!) { cartCreate(input: { lines: [{ quantity: 1, merchandiseId: $variantId }] }) { cart { ${CART_FRAGMENT} } userErrors { message } } }`, { variantId: variantGid });
...
const { data } = await gql(`mutation cartLinesAdd($cartId: ID!, $variantId: ID!) { cartLinesAdd(cartId: $cartId, lines: [{ quantity: 1, merchandiseId: $variantId }]) { cart { ${CART_FRAGMENT} } userErrors { message } } }`, { cartId, variantId: variantGid });
...
const { data } = await gql(`mutation cartLinesUpdate($cartId: ID!, $lineId: ID!, $qty: Int!) { cartLinesUpdate(cartId: $cartId, lines: [{ id: $lineId, quantity: $qty }]) { cart { ${CART_FRAGMENT} } userErrors { message } } }`, { cartId, lineId, qty: newQty });
...
const { data } = await gql(`mutation cartLinesRemove($cartId: ID!, $lineIds: [ID!]!) { cartLinesRemove(cartId: $cartId, lineIds: $lineIds) { cart { ${CART_FRAGMENT} } userErrors { message } } }`, { cartId, lineIds: [lineId] });
...
style.textContent = `.tidal-cart-overlay { position: fixed; inset: 0; background: rgba(20,20,20,0.25); opacity: 0; pointer-events: none; transition: opacity 0.3s; z-index: 998; } .tidal-cart-overlay.is-open { opacity: 1; pointer-events: auto; } .tidal-cart-drawer { position: fixed; top: 0; right: 0; height: 100vh; height: 100dvh; width: 360px; max-width: 92vw; background: var(--cream, #f4ede2); transform: translateX(100%); transition: transform 0.35s cubic-bezier(.2,.7,.2,1); z-index: 999; display: flex; flex-direction: column; box-shadow: -2px 0 24px rgba(0,0,0,0.04); } .tidal-cart-drawer.is-open { transform: translateX(0); } .tidal-cart-header { display: flex; align-items: center; justify-content: space-between; padding: 22px 24px 18px; background: var(--cream, #f4ede2); } .tidal-cart-header h2 { margin: 0; font-family: 'Italiana', serif; font-size: 14px; color: #2a2a2a; letter-spacing: 0.26em; text-transform: uppercase; font-weight: 400; } .tidal-cart-close { background: none; border: 0; padding: 4px; cursor: pointer; color: #2a2a2a; line-height: 0; opacity: 0.65; transition: opacity 0.15s; } .tidal-cart-close:hover { opacity: 1; } .tidal-cart-close svg { display: block; width: 14px; height: 14px; } .tidal-cart-body { flex: 1; overflow-y: auto; padding: 0 24px; background: var(--cream, #f4ede2); } .tidal-cart-empty { text-align: center; padding: 80px 20px; font-family: 'Cormorant Garamond', serif; font-style: italic; font-size: 15px; color: rgba(42,42,42,0.45); } .tidal-cart-line { display: grid; grid-template-columns: 56px 1fr auto; gap: 14px; padding: 18px 0; border-bottom: 1px solid rgba(42,42,42,0.08); } .tidal-cart-line:first-child { padding-top: 6px; } .tidal-cart-line:last-child { border-bottom: 0; } .tidal-cart-line-img { width: 56px; height: 72px; background: rgba(42,42,42,0.04); object-fit: cover; display: block; } .tidal-cart-line-info { display: flex; flex-direction: column; gap: 3px; min-width: 0; } .tidal-cart-line-title { font-family: 'Italiana', serif; font-size: 13px; color: #2a2a2a; letter-spacing: 0.04em; line-height: 1.25; } .tidal-cart-line-meta { font-size: 9px; letter-spacing: 0.2em; text-transform: uppercase; color: rgba(42,42,42,0.55); font-family: 'Inter', sans-serif; line-height: 1.4; } .tidal-cart-line-qty { display: inline-flex; align-items: center; gap: 10px; margin-top: 8px; } .tidal-cart-qty-btn { width: 20px; height: 20px; border: 1px solid rgba(42,42,42,0.18); background: transparent; cursor: pointer; font-size: 11px; color: #2a2a2a; display: inline-flex; align-items: center; justify-content: center; padding: 0; transition: background 0.15s, border-color 0.15s; line-height: 0; } .tidal-cart-qty-btn:hover { background: rgba(42,42,42,0.04); border-color: rgba(42,42,42,0.35); } .tidal-cart-qty-num { min-width: 14px; text-align: center; font-size: 11px; color: #2a2a2a; font-family: 'Inter', sans-serif; letter-spacing: 0.05em; } .tidal-cart-line-right { display: flex; flex-direction: column; align-items: flex-end; justify-content: space-between; gap: 6px; } .tidal-cart-line-price { font-family: 'Italiana', serif; font-size: 13px; color: #2a2a2a; letter-spacing: 0.04em; } .tidal-cart-line-remove { background: none; border: 0; padding: 2px 6px; cursor: pointer; color: rgba(42,42,42,0.35); font-size: 16px; line-height: 1; transition: color 0.15s; font-family: 'Inter', sans-serif; font-weight: 300; } .tidal-cart-line-remove:hover { color: var(--coral, #c8553d); } .tidal-cart-footer { padding: 18px 24px 22px; background: var(--cream, #f4ede2); border-top: 1px solid rgba(42,42,42,0.08); flex-shrink: 0; } .tidal-cart-subtotal { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 16px; font-family: 'Inter', sans-serif; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: rgba(42,42,42,0.7); font-weight: 400; } .tidal-cart-subtotal-amount { font-family: 'Italiana', serif; font-size: 18px; letter-spacing: 0.04em; text-transform: none; color: #2a2a2a; } .tidal-cart-checkout { display: block; width: 100%; max-width: 100%; padding: 14px 16px; background: var(--navy, #0f1d3a); color: var(--cream, #f4ede2); border: 0; cursor: pointer; text-align: center; text-decoration: none; font-family: 'Inter', sans-serif; font-size: 11px; letter-spacing: 0.28em; text-transform: uppercase; font-weight: 400; box-sizing: border-box; transition: background 0.18s; line-height: 1.2; } .tidal-cart-checkout:hover { background: #1c2c4d; } .tidal-cart-checkout:disabled { opacity: 0.35; cursor: not-allowed; } .tidal-cart-continue { display: block; text-align: center; padding-top: 14px; font-family: 'Cormorant Garamond', serif; font-style: italic; font-size: 13px; color: rgba(42,42,42,0.5); background: none; border: 0; cursor: pointer; width: 100%; } .tidal-cart-continue:hover { color: #2a2a2a; } @media (max-width: 480px) { .tidal-cart-drawer { width: 100vw; max-width: 100vw; } .tidal-cart-header { padding: 16px 18px 12px; } .tidal-cart-body { padding: 0 18px; } .tidal-cart-footer { padding: 14px 18px calc(20px + env(safe-area-inset-bottom, 0px)); } .tidal-cart-checkout { padding: 14px 14px; font-size: 10px; letter-spacing: 0.26em; } }`;
document.head.appendChild(style);
}
...
drawer.innerHTML = `<header class="tidal-cart-header"> <h2>Your Bag</h2> <button class="tidal-cart-close" aria-label="Close bag"> <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"> <path d="M6 6l12 12"/><path d="M6 18L18 6"/> </svg> </button> </header> <div class="tidal-cart-body" id="tidal-cart-body"> <div class="tidal-cart-empty">Your bag is empty.</div> </div> <footer class="tidal-cart-footer" id="tidal-cart-footer" style="display:none;"> <div class="tidal-cart-subtotal"> <span>Subtotal</span> <span class="tidal-cart-subtotal-amount" id="tidal-cart-subtotal">£0.00</span> </div> <a class="tidal-cart-checkout" id="tidal-cart-checkout" href="#">Checkout</a> <button class="tidal-cart-continue" type="button" id="tidal-cart-continue">Continue shopping</button> </footer>`;
...
return `<div class="tidal-cart-line" data-line-id="${line.id}"> ${imgTag} <div class="tidal-cart-line-info"> <div class="tidal-cart-line-title">${v.product.title}</div> <div class="tidal-cart-line-meta">${opts}</div> <div class="tidal-cart-line-qty"> <button class="tidal-cart-qty-btn" data-action="decrease">−</button> <span class="tidal-cart-qty-num">${line.quantity}</span> <button class="tidal-cart-qty-btn" data-action="increase">+</button> </div> </div> <div class="tidal-cart-line-right"> <button class="tidal-cart-line-remove" data-action="remove" aria-label="Remove">×</button> <div class="tidal-cart-line-price">${fmtMoney(v.price.amount * line.quantity, v.price.currencyCode)}</div> </div> </div>`;
}).join(’’);
